...
# --------------------------------------------------------------------------------
# NEW HELPER: Split-Aware Resizing / Masking
# --------------------------------------------------------------------------------
# --------------------------------------------------------------------------------
# DEBUGGING HELPER: Split-Aware Resizing with ENERGY LOGGING
# --------------------------------------------------------------------------------
def dynamic_resize_lora(model, state_dict):
    config = model.config
    cut_layer = config.cut_layer
    # Default to lora_attn_dim if specific eval rank is not set
    target_client_rank = getattr(config, 'client_eval_rank', config.lora_attn_dim)
    if target_client_rank is None: target_client_rank = config.lora_attn_dim

    if config.lora_attn_dim == 0:
        return {k: v for k, v in state_dict.items() if "lora_" not in k}

    print(f"\n{'='*100}")
    print(f"DEBUG: FULL LAYER AUDIT")
    print(f"Config: Cut Layer={cut_layer} | Container={config.lora_attn_dim} | Client Target={target_client_rank}")
    print(f"{'='*100}")
    print(f"{'L#':<4} | {'Role':<8} | {'Param':<8} | {'Norm Before':<12} | {'Norm After':<12} | {'Status'}")
    print(f"{'-'*100}")

    # Helper: "Mask" a tensor
    def _mask_tensor_to_rank(tensor, keep_rank, is_A):
        if is_A:
            # LoRA_A [2*r, dim]
            full_rank = tensor.shape[0] // 2
            if keep_rank >= full_rank: return tensor
            chunks = torch.chunk(tensor, 2, dim=0)
            masked_chunks = []
            for c in chunks:
                mask = torch.ones_like(c)
                mask[keep_rank:, :] = 0
                masked_chunks.append(c * mask)
            return torch.cat(masked_chunks, dim=0)
        else:
            # LoRA_B [dim, r]
            full_rank = tensor.shape[1]
            if keep_rank >= full_rank: return tensor
    ...
    # Helper: "Pad" a tensor
    def _pad_tensor(tensor, target_rank, is_A):
        if is_A:
            current_rank = tensor.shape[0] // 2
            if current_rank >= target_rank: return tensor
            chunks = torch.chunk(tensor, 2, dim=0)
            padded = [F.pad(c, (0, 0, 0, target_rank - current_rank)) for c in chunks]
            return torch.cat(padded, dim=0)
        else:
            current_rank = tensor.shape[1]
            if current_rank >= target_rank: return tensor
            return F.pad(tensor, (0, target_rank - current_rank, 0, 0))

    # Helper: Get Energy
    def get_norm(t):
        return t.abs().sum().item()

    # Sort keys to ensure we print Layer 0, then Layer 1, etc.
    def extract_layer_idx(k):
        parts = k.split('.')
        if 'h' in parts:
            try:
                return int(parts[parts.index('h') + 1])
            except: return 999
        return 999
    
    sorted_keys = sorted(state_dict.keys(), key=extract_layer_idx)
    prev_layer = -1
    ...
    for key in sorted_keys:
        ...
        if layer_idx == 999: continue

        # Visual separator
        if layer_idx != prev_layer and prev_layer != -1:
            print(f"{'-'*100}")
        prev_layer = layer_idx

        # Logic
        is_server = (layer_idx >= cut_layer)
        is_A = "lora_A" in key
        param_type = "A" if is_A else "B"
        norm_before = get_norm(tensor)
        
        # APPLY LOGIC
        if is_server:
            # SERVER: Pad to fit container, no masking
            new_tensor = _pad_tensor(tensor, config.lora_attn_dim, is_A=is_A)
            status = "SERVER (Kept)"
        else:
            # CLIENT: Pad to fit container -> Mask to simulate rank
            padded = _pad_tensor(tensor, config.lora_attn_dim, is_A=is_A)
            
            # Only mask if target < container size
            if target_client_rank < config.lora_attn_dim:
                new_tensor = _mask_tensor_to_rank(padded, target_client_rank, is_A=is_A)
                status = f"CLIENT (Masked to {target_client_rank})"
            else:
                new_tensor = padded
                status = "CLIENT (Full)"
        
        state_dict[key] = new_tensor
        norm_after = get_norm(new_tensor)
        
        # Print Row (Only print if it changes or is interesting)
        # We print EVERYTHING now to be sure
        role = "SERVER" if is_server else "CLIENT"
        print(f"{layer_idx:<4} | {role:<8} | {param_type:<8} | {norm_before:<12.4f} | {norm_after:<12.4f} | {status}")
...
class GPT2Config(object):
    def __init__(
        ...
        lora_r_dropout=0.0,
        fix_dropout=0.0,
        cut_layer=3,                 # Already exists
        client_eval_rank=None        # <--- NEW ARGUMENT
    ):
        ...
        self.client_eval_rank = client_eval_rank # Store it
...
class GPT2LMModel(nn.Module):
    ...
    def load_weight(self, state_dict):
        ...
        state_dict = dynamic_resize_lora(self, state_dict)
