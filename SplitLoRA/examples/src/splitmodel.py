...
def debug_lora_integrity(tag, tensor_name, tensor):
    ...
    # tensor shape is [2*rank, dim] (assuming enable_lora=[T,F,T])
...
class GPT2Model_Client(nn.Module):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([copy.deepcopy(block) for _ in range(config.cut_layer)])
...
class GPT2Model_Server(nn.Module):
    def __init__(self, config):
        ...
        self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        block = Block(config.n_ctx, config, scale=True)
        self.h = nn.ModuleList([copy.deepcopy(block) for _ in range(config.n_layer-config.cut_layer)])
...
# --------------------------------------------------------------------------------
# Helper to dynamically resize LoRA layers (Required for FLoRA checkpoint loading)
# --------------------------------------------------------------------------------
def dynamic_resize_lora(model, state_dict):
    """
    Adjusts LoRA tensors in the checkpoint to match the model's configured rank.
    Handles 'MergedLinear' correctly by identifying Q and V blocks.
    """
    config = model.config
    ...
    # --- DEBUG: Check Integrity BEFORE Resize ---
    print("\n=== DEBUG: CHECKPOINT LOADING (Before Resize) ===")
    for k, v in state_dict.items():
        if "h.0.attn.c_attn.lora_A" in k:
            debug_lora_integrity("CKPT_LOAD_RAW", k, v)

    check_key = None
    for k in state_dict.keys():
        if k.endswith("attn.c_attn.lora_A"):
            check_key = k
            break
            
    if check_key:
        saved_weight = state_dict[check_key]
        # We assume MergedLinear structure for GPT2 (Q+V)
        saved_r = saved_weight.shape[0] // 2
        target_r = config.lora_attn_dim

        if saved_r != target_r:
            print(f"WARN: Rank mismatch detected! Config r={target_r}, Checkpoint r={saved_r}.")

            def _resize_tensor(tensor, target_rank, is_A):
                if is_A:
                    # LoRA_A: [2*r, dim]. Structure: [Query_Rows ... Value_Rows ...]
                    current_rank = tensor.shape[0] // 2
                    
                    # 1. Split into Query and Value chunks
                    chunks = torch.chunk(tensor, 2, dim=0)
                    new_chunks = []
                    
                    for chunk in chunks:
                        # chunk shape: [current_rank, dim]
                        if target_rank > current_rank:
                            # Pad rows (bottom)
                            pad_rows = target_rank - current_rank
                            new_chunk = F.pad(chunk, (0, 0, 0, pad_rows))
                        else:
                            # Truncate rows
                            new_chunk = chunk[:target_rank, :]
                        new_chunks.append(new_chunk)
                    
                    # 2. Stack them back: [New_Query; New_Value]
                    return torch.cat(new_chunks, dim=0)
                else:
                    # LoRA_B: [dim, r]. Just pad/truncate columns.
            ...
            for key, tensor in list(state_dict.items()):
                if key.endswith("lora_A"):
                    state_dict[key] = _resize_tensor(tensor, target_r, is_A=True)
                elif key.endswith("lora_B"):
                    state_dict[key] = _resize_tensor(tensor, target_r, is_A=False)

    # --- DEBUG: Check Integrity AFTER Resize ---
    print("\n=== DEBUG: CHECKPOINT LOADING (After Resize) ===")
    for k, v in state_dict.items():
        if "h.0.attn.c_attn.lora_A" in k:
            debug_lora_integrity("CKPT_LOAD_RESIZED", k, v)
...
class GPT2LMModel_Client(nn.Module):
    ...
    def load_weight(self, state_dict):
        ...
        state_dict = dynamic_resize_lora(self.transformer_Client, state_dict)
        old_keys = []
        new_keys = []
...
class GPT2LMModel_Server(nn.Module):
    ...
    def load_weight(self, state_dict):
        ...
        state_dict = dynamic_resize_lora(self.transformer_Server, state_dict)
        old_keys = []
        new_keys = []
        for key in list(state_dict.keys()):
            new_key = None
        ...
        for key, value in state_dict.items():
            if key.startswith('h.'):
                ...
                cut_layer = self.transformer_Server.config.cut_layer
                new_key = '.'.join(['h', str(layer_idx - cut_layer)] + parts[2:])
